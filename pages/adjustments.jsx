...
function DepreciationSchedulePage({ open, onClose, activeScheduleType, onScheduleTypeChange, suggestionsCount, reviewState, onReviewStateChange, viewMode, onToggleMode, adjComments = {}, onAddAdjComment }) {
  ...
  const _dpStatusBadge = (item) => {
    ...
    if (item.status === "fully_depreciated") return <span style={{ display: "inline-flex", alignItems: "center", background: T.colorWarningBg, color: T.colorWarning, borderRadius: 4, padding: "2px 8px", fontSize: 12, fontWeight: 500, lineHeight: "17px" }}>Fully depreciated</span>;
    if (item.status === "disposed") return <span style={{ display: "inline-flex", alignItems: "center", background: "#ECECEC", color: "#757980", borderRadius: 4, padding: "2px 8px", fontSize: 12, fontWeight: 500, lineHeight: "17px", whiteSpace: "nowrap" }}>{"Disposed " + item.disposalDate}</span>;
    return null;
  };
